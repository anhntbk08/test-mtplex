use crate::{
    error::ErrorCode,
    state::{GatingConfig, MarketState, MarketType, SellingResourceState, MINIMUM_BALANCE_FOR_SYSTEM_ACCS},
    utils::*,
    CreateMarket,
};
use anchor_lang::{
...
impl<'info> CreateMarket<'info> {
    pub fn process(
        ...
        if allowed_tokens.len() == 0 || (vault_token_accounts.len() != allowed_tokens.len()) {
        ...
        market.vault_token_accounts = vault_token_accounts;
