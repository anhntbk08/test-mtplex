...
pub mod mpl_stoneage {
    ...
    pub fn init_selling_resource<'info>(
        ...
        ctx.accounts
            .process(master_edition_bump, vault_owner_bump)
    }
...
#[derive(Accounts)]
#[instruction(master_edition_bump:u8, vault_owner_bump: u8)]
...
#[derive(Accounts)]
#[instruction(treasury_owner_bump: u8, name: String, description: String, mutable: bool, allowed_tokens: Vec<Pubkey>, vault_token_accounts: Vec<Pubkey>, pieces_in_one_wallet: Option<u64>, start_date: u64, end_date: Option<u64>, gating_config: Option<GatingConfig>)]
...
pub struct Buy<'info> {
    ...
    // this account for storing token mint - will be validated if this in the allowed list
    /// CHECK: checked in program
    buyer_resource_mint: Box<Account<'info, Mint>>,
    ...
    #[account(init_if_needed, seeds=[HISTORY_PREFIX.as_bytes(), buyer_wallet.key().as_ref(), market.key().as_ref()], bump, payer=buyer_wallet, space=TradeHistory::LEN)]
